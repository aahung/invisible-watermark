...
class DigitalWatermarking {
  ...
  static existsFilePath(filePath) {
    if (!fs.existsSync(filePath)) {
      throw new Error(`not file in ${filePath}`);
    }
  }
  ...
  static async transformImageBufferWithText(
    srcBuffer,
    watermarkText,
    fontSize
  ) {
    DigitalWatermarking.isBuffer(srcBuffer);
    return await lib.transformImageWithText(srcBuffer, watermarkText, fontSize);
  }

  static async getTextFormImage(enCodeFileName, deCodeFileName) {
    const enCodeFilePath = DigitalWatermarking.getAbsoluteFilePath(
      enCodeFileName
    );
  ...
  static async getTextFormImageBuffer(enCodeBuffer) {
    ...
    return await lib.getTextFormImage(enCodeBuffer);
  }
}
